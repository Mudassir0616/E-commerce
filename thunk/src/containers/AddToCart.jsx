...
const AddToCart = () => {
  ...
  const renderCartItems = cart?.map((item, index) =>(
    <div className="cart-items" key={item?.id}>
      ...
      <div className="item-details">
        <h2>{item?.title} <h5 style={{color:'darkblue'}}>{item.brand}</h5></h2>
        <p>{item.description}</p>
        <h4>Rating - <span style={{color:'#cd9042'}}>{item.rating} &#9733;</span></h4>
        ...
        <button className='item-btn' onClick={()=> handleDelete(index)}>Remove</button>
      </div>
  ...
  return (
    <div className='render-cart'>
      ...
      {renderCartItems?.length == 0 ?(<h2 className='no-item'>There are 0 Items in the <ShoppingCartCheckoutIcon sx={{fontSize:'30px'}}/></h2>):''}
      
      
      {renderCartItems}
      </div>
      <div className="pricing">
        <h4>Your Sub Total  Item <ShoppingCartIcon/>{cart.length} : <br /><br />
        <span style={{color:'#cd9042',fontSize:'19px'}}>
          {sum}$
        </span> </h4><br />
        
        <Link to={'/checkout'}>
        <button title='Shop Again !!' className='chck-btn'>Proceed to Checkout</button>
        </Link>
      </div>
      </>
      ): (
